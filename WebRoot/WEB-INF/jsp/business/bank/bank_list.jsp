...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="bank/BanklistPage.do" method="post" name="residenceForm" id="residenceForm">
			<table>
				<tr>
					<td>
		...
		<tbody>
				...
				<c:choose>
					<c:when test="${not empty bankList}">
						<c:if test="${QX.cha == 1 }">
						<c:forEach items="${bankList}" var="rlist" varStatus="vs">
									
							<tr>
								<td class='center' style="width: 30px;">${vs.index+1}</td>
								<td>${rlist.bank_name }</td>
								<td>${rlist.card_number }</td>
								<td>${rlist.card_huming }</td>
								<td>
								<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${rlist.bank_id }')" ><i class='icon-edit'></i></a>
								<a class='btn btn-mini btn-danger' title="删除"  onclick="del('${rlist.bank_id }',true)"><i class='icon-trash'></i></a>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>bank/goAddPage.do';
			 diag.Width = 535;
			 diag.Height = 405;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function edit(tagID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>bank/edit.do?tagID='+tagID;
			 diag.Width = 535;
			 diag.Height = 405;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(tagID,msg){
			bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>bank/del.do?tagID='+tagID;
