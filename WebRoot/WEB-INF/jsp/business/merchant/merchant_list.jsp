...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="merchant/MerChantlistPage.do" method="post" name="residenceForm" id="residenceForm">
			<table>
				<tr>
					<td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>merchant/goAddPage.do';
			 diag.Width = 535;
			 diag.Height = 405;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function edit(tagID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>merchant/edit.do?tagID='+tagID;
			 diag.Width = 535;
			 diag.Height = 405;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(tagID,msg){
			bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>merchant/del.do?tagID='+tagID;
