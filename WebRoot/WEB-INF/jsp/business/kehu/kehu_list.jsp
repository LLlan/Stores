					...
					<c:when test="${not empty kehuList}">
						...
						<c:forEach items="${kehuList}" var="rlist" varStatus="vs">
							...
							<tr>
								...
								<td>${rlist.contact }</td>
								<td>
								<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${rlist.kehu_id }')" ><i class='icon-edit'></i></a>
								<a class='btn btn-mini btn-danger' title="删除"  onclick="del('${rlist.kehu_id }',true)"><i class='icon-trash'></i></a>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(tagID,msg){
			bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>kehu/del.do?tagID='+tagID;
