...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="jinhuo/jinhuoListPage.do" method="post" name="residenceForm" id="residenceForm">
			<table>
				<tr>
					<td>
		...
		<tbody>
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty jinhuoList}">
						<c:if test="${QX.cha == 1 }">
						<c:forEach items="${jinhuoList}" var="rlist" varStatus="vs">
							...
							<%-- 	<td class='center' style="width: 30px;">
								...
								<td>${rlist.goods_name}</td>
								<td>${rlist.ggxh }</td>
								<td>${rlist.danwei }</td>
								<td>${rlist.danjia }</td>
								<td>${rlist.shuliang }</td>
								<td>${rlist.money }</td>
								<td>${rlist.money_dx }</td>
								<td>${rlist.jinhuo_time }</td>
								<td>${rlist.remark }</td>
								<td>${rlist.caozuoyuan }</td>
								<td>
								<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${rlist.sp_id }')" ><i class='icon-edit'></i></a>
								<a class='btn btn-mini btn-danger' title="删除"  onclick="del('${rlist.sp_id }','${rlist.spmc_id }','${rlist.shuliang }')"><i class='icon-trash'></i></a>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>jinhuo/goAddPage.do';
			 diag.Width = 575;
			 diag.Height = 485;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function edit(tagID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>jinhuo/edit.do?tagID='+tagID;
			 diag.Width = 575;
			 diag.Height = 485;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(tagID,spmc_id,shuliang){
			 bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>jinhuo/del.do?tagID='+tagID+'&spmc_id='+spmc_id+'&shuliang='+shuliang;
