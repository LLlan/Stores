...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="xiaoshou/xiaoshouListPage.do" method="post" name="residenceForm" id="residenceForm">
			<table>
				<tr>
					<td>
		...
		<tbody>
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty xiaoshouList}">
						<c:if test="${QX.cha == 1 }">
						<c:forEach items="${xiaoshouList}" var="rlist" varStatus="vs">
									
							<tr>
								<td class='center' style="width: 30px;">${vs.index+1}</td>
								<td>${rlist.kehu_name}</td>
								<td>${rlist.xiaoshou_time }</td>
								<td>${rlist.all_totol }</td>
								<td>${rlist.jbr }</td>
								<td>${rlist.remark }</td>
								<td><a href="<%=basePath%>/xiaoshou/xiaoshouDetail.do?xiaoshou_id=${rlist.xiaoshou_id}" title="查看详情"><i class="icon-arrow-right  icon-on-right"></i>&nbsp;查看</a></td>
								<td>
								<%-- <a class='btn btn-mini btn-info' title="编辑" onclick="edit('${rlist.xiaoshou_id }')" ><i class='icon-edit'></i></a> --%>
								<a class='btn btn-mini btn-danger' title="删除"  onclick="del('${rlist.xiaoshou_id }',true)"><i class='icon-trash'></i></a>
								</td>
							</tr>
						...
						<c:if test="${QX.cha == 0 }">
							<tr>
								<td colspan="20" class="center">您无权查看</td>
							</tr>
						</c:if>
					</c:when>
					<c:otherwise>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>xiaoshou/goAddPage.do';
			 diag.Width = 775;
			 diag.Height = 585;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function edit(tagID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>xiaoshou/edit.do?tagID='+tagID;
			 diag.Width = 775;
			 diag.Height = 585;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(tagID,msg){
			bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>xiaoshou/del.do?tagID='+tagID;
