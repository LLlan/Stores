...
<div class="container-fluid" id="main-container">
	<div id="page-content" class="clearfix">
		<div class="row-fluid">
			<div class="row-fluid">
				<!-- 检索  -->
				<form action="api/mainImg/mainImglistPage.do" method="post" id="Form">
					<table>
						<tr>
							<!-- 
							<td>
								...
								<c:forEach items="${list}" var="va" varStatus="vs">
									<tr>
										<td class='center' style="width: 30px;">
											<label>
												<input type='checkbox' name='ids' value="${va.main_img_id }"/>
												<span class="lbl"></span>
											</label>
										</td>
										<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td>
											<img style="width: 100px;" alt="" src="${va.img_path }">
										</td>
										<td>${va.status=='1'?'使用中':'已停用' }</td>
										<td>${va.type=='1'?'服务中心':va.type=='2'?'新闻中心':'工种中心' }</td>
										<td style="width: 60px;">
											<div class='hidden-phone visible-desktop btn-group'>
												<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${va.main_img_id }');"><i class='icon-edit'></i></a>
												<a class='btn btn-mini btn-danger' title="删除" onclick="del('${va.main_img_id }');"><i class='icon-trash'></i></a>
											</div>
										</td>
									</tr>
								</c:forEach>
...
<script type="text/javascript">
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>api/mainImg/toAdd.do';
		 diag.Width = 500;
		 diag.Height = 350;
		 diag.CancelEvent = function(){ //关闭事件
	...
	//修改
	function edit(main_img_id){
		 top.jzts();
		 var diag = new top.Dialog();
		 diag.Drag=true;
		 diag.Title ="资料";
		 diag.URL = '<%=basePath%>api/mainImg/toEdit.do?main_img_id='+main_img_id;
		 diag.Width = 500;
		 diag.Height = 350;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	//删除
	function del(main_img_id){
		bootbox.confirm("确定要执行删除操作吗?", function(result) {
			if(result) {
				top.jzts();
				var url = "<%=basePath%>api/mainImg/delete.do?ids="+main_img_id;
		...
		bootbox.confirm(msg, function(result) {
			if(result) {
				...
				if(str==''){
					...
					if(msg == '确定要删除选中的数据吗?'){
						top.jzts();
						var url = "<%=basePath%>api/mainImg/delete.do?ids="+str;
						$.get(url,function(data){
							nextPage("${page.currentPage}");
						});
						/*$.ajax({
							type: "POST",
							url: '<%=basePath%>api/mainImg/delete.do',
					    	data: {ids:str},
							dataType:'json',
