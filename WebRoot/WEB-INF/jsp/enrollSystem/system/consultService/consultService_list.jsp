...
<div class="container-fluid" id="main-container">
	<div id="page-content" class="clearfix">
		<div class="row-fluid">
			<div class="row-fluid">
				<!-- 检索  -->
				<form action="api/customerOperation/getConsultServicelistPage.do" method="post" id="Form">
					<table>
						<tr>
							<td>
							...
							<td style="vertical-align:top;"> 
							 	<select class="chzn-select" name="status" data-placeholder="请选择状态" style="vertical-align:top;width: 120px;">
								<option value=""></option>
								...
								<c:forEach items="${list}" var="va" varStatus="vs">
									<tr>
										<td class='center' style="width: 30px;">
											<label>
												<input type='checkbox' name='ids' value="${va.consult_service_id }"/>
												<span class="lbl"></span>
											</label>
										</td>
										<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td>${va.consult_person_name }</td>
										<td>${va.contact_number }</td>
										<td>${va.content }</td>
										<td>${va.consult_time }</td>
										<td>${va.feedback_time==null?"尚未反馈":va.feedback_time }</td>
										<td>${va.status=="1"?"已处理":"未处理" }</td>
										<td style="width: 60px;">
											<div class='hidden-phone visible-desktop btn-group'>
												<a style="width: 50px;" class='btn btn-mini btn-info' title="编辑" onclick="edit('${va.consult_service_id }');"><i class='icon-edit'></i></a>
												<!--  
												<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${va.consult_service_id }');"><i class='icon-edit'></i></a>
												<a class='btn btn-mini btn-danger' title="删除" onclick="del('${va.consult_service_id }');"><i class='icon-trash'></i></a>
												-->
											</div>
										</td>
									</tr>
...
<script type="text/javascript">
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>api/expertLibrary/toAdd.do';
		 diag.Width = 500;
		 diag.Height = 350;
		 diag.CancelEvent = function(){ //关闭事件
	...
	function edit(id){
		 ...
		 diag.Drag=true;
		 diag.Title ="资料";
		 diag.URL = '<%=basePath%>api/expertLibrary/toEdit.do?expert_library_id='+id;
		 diag.Width = 500;
		 diag.Height = 350;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function del(id){
		bootbox.confirm("确定要执行删除操作吗?", function(result) {
			if(result) {
				top.jzts();
				var url = "<%=basePath%>api/expertLibrary/delete.do?ids="+id;
		...
		bootbox.confirm(msg, function(result) {
			if(result) {
				...
				if(str==''){
					...
					if(msg == '确定要删除选中的数据吗?'){
						top.jzts();
						var url = "<%=basePath%>api/expertLibrary/delete.do?ids="+str;
						$.get(url,function(data){
							nextPage("${page.currentPage}");
						});
						/*$.ajax({
							type: "POST",
							url: '<%=basePath%>api/trainCategory/delete.do',
					    	data: {ids:str},
							dataType:'json',
